...
class QuizManager {
    ...
    private var alreadyAnswered: [Int] = []
    ...
    func refreshQuiz() {
        var randomIndex: Int
        repeat {
            randomIndex = Int(arc4random_uniform(UInt32(quizes.count)))
            if self.alreadyAnswered.count == quizes.count {
                break
            }
        } while self.alreadyAnswered.contains(randomIndex)
        self.alreadyAnswered.append(randomIndex)
        let quizData = quizes[randomIndex]
        if self.alreadyAnswered.count == quizes.count {
            quiz = Quiz(question: quizData.question, options: [], correctedAnswer: quizData.correctAnswer)
        } else {
